...
How it works
Assistant — answers the user's query directly from its own knowledge.
Web Search Assistant — searches the web (via Serper) and answers based on the results.
Entry Agent — writes the query and both answers to `answers.txt`, and returns both answers to be shown in the UI.
The three agents run in order using CrewAI's `Process.sequential`, with the Entry Agent's task receiving the outputs of the first two tasks as context.
Setup
...
```
   ...
   ```
Open the app at `http://localhost:8501`, enter a query, and click Submit.
Output
